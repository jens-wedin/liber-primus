...
% HistoCrypt submission version (ACL/EACL author-kit template).
% ---------------------------------------------------------------------------
% COMPILE WITH XeLaTeX or LuaLaTeX (the paper prints Anglo-Saxon runic glyphs).
% Tectonic works out of the box:  tectonic liber-primus-cryptodiagnosis-histocrypt.tex
% Needs histocrypt.sty (this dir, from the HistoCrypt author kit) and a font
% covering the Unicode Runic block, e.g. "Noto Sans Runic" (free, Google Noto).
% The body stays Times-like; fontspec is used ONLY for the rune glyphs, so the
% ACL two-column layout is unchanged. If no runic font is found the document
% still compiles (the runes fall back to the main font).
% ---------------------------------------------------------------------------
\documentclass[11pt]{article}
\usepackage{histocrypt}
\usepackage{amsmath}

% --- Fonts: keep the ACL Times look, add Unicode + a runic face -------------
\usepackage{fontspec}
...
\author{Jens Wedin \\
  ...
  {\tt jens@studiomanfred.com} \\
  {\tt github.com/jens-wedin/liber-primus} \\}
...
We measured the doublet rate at 0.66\% (86 equal-adjacent pairs in 12,934), against a random expectation of 1/29 = 3.45\%. The indicators are pairwise independent, so the variance is exact; the deficiency is z ≈ −17.4. Differences at lags 2 through 8 are null. The non-zero first differences are uniform to p ≈ 0.04. So the anomaly is a \emph{pure lag-1 effect} and nothing else.
...
A hard no-repeat rule would leave zero doublets. The stream has 86. We show these are not transcription noise but signal. A second transcription reproduces all 86, so copy error at the required rate is not supported. The mechanism is that the rule is \emph{soft}: it keeps a would-be doublet with a fixed acceptance probability. Fitting that probability on our stream,
\[ p_{\text{keep}} = \frac{86\ \text{observed}}{12{,}934\ \text{pairs} / 29\ \text{expected}} = 0.193, \]
and a simulation at p = 0.19 reproduces both the flat IoC and the 0.66\% rate. A parallel project (below) fitted the same figure (≈ 0.18) independently. The 86 doublets are the filter's acceptance leak. Combinatorially the construction is a Smirnov word / Carlitz composition --- a sequence with a bias against equal neighbours. This is orthogonal to the resolution distribution, which we found uniform (a deterministic bump or nearest-value nudge is ruled out).

\section{Methodology: control-validated elimination}

An elimination claim has the form ``if cipher X were the answer, we would have seen it.'' That claim is testable, and if it is not tested the negative is worthless. A self-audit of our own early work found both failure modes, so we state the methodology as the paper's second contribution.
...
This diagnosis stands on a decade of open community work. I thank the maintainers of the transcriptions this paper depends on --- \texttt{scream314} (the primary rune transcription) and \texttt{rtkd} (the independent cross-check and the byte-string rendering of the code pages) --- and the CicadaSolvers and Uncovering Cicada communities for the frequency analysis and the puzzle history. Two independent solver projects were essential to the cross-validation: \texttt{relikd}'s \emph{LiberPrayground}, whose finished InterruptDB let me verify the interrupter and modulo families without a 38-hour rebuild, and \texttt{Dukotah}'s \emph{cicada3301}, whose control-validated ledger and soft-rejection fit corroborate the central finding on the same ciphertext object. Any errors are my own.

\begin{thebibliography}{}

\bibitem[\protect\citename{Bean}2021]{bean2021}
Robert Bean. 2021.
\newblock Cryptodiagnosis of ``Kryptos K4''.
\newblock In {\em Proceedings of the 4th International Conference on Historical Cryptology (HistoCrypt 2021)}, volume 183 of {\em Link\"oping Electronic Conference Proceedings}, pages 25--33. Link\"oping University Electronic Press. DOI: 10.3384/ecp183153.

\bibitem[\protect\citename{Lasry}2018]{lasry2018}
George Lasry. 2018.
\newblock {\em A Methodology for the Cryptanalysis of Classical Ciphers with Search Metaheuristics}.
\newblock Kassel University Press.

\bibitem[\protect\citename{Oranchak et~al.}2024]{oranchak2024}
David Oranchak, Sam Blake, and Jarl Van Eycke. 2024.
\newblock The solution of the Zodiac Killer's 340-character cipher.
\newblock arXiv:2403.17350.

\bibitem[\protect\citename{Shannon}1949]{shannon1949}
Claude E. Shannon. 1949.
\newblock Communication theory of secrecy systems.
\newblock {\em Bell System Technical Journal}, 28(4):656--715.

\bibitem[\protect\citename{Speer}2022]{speer2022}
Robyn Speer. 2022.
\newblock rspeer/wordfreq (v3.0).
\newblock Zenodo. DOI: 10.5281/zenodo.7199437.

\bibitem[\protect\citename{Wedin}2026]{wedin2026}
Jens Wedin. 2026.
\newblock Liber Primus cryptanalysis toolkit.
\newblock \url{https://github.com/jens-wedin/liber-primus}.

\bibitem[\protect\citename{Cicada solver community}]{community}
Cicada 3301 solver community.
\newblock Transcriptions and archives: {\tt scream314/cicada3301}, {\tt rtkd/iddqd}, {\tt krisyotam/cicada3301}. Independent solver projects: {\tt relikd/LiberPrayground} (the InterruptDB, interrupter and modulo sweeps) and {\tt Dukotah/cicada3301} (a parallel control-validated project). Reference material: the Uncovering Cicada wiki, \url{https://uncovering-cicada.fandom.com}, and the CicadaSolvers briefing, \url{https://www.cicadasolvers.com/quickstart/}.

\end{thebibliography}

\appendix

\section{Gematria Primus}

The 29-rune alphabet, in its canonical order, is given in Table~\ref{tab:gp}. Each rune maps to one English letter or digraph and to the $n$th prime in ascending order. Index $i$ is the value used for the modular arithmetic in the body (c = p + k mod 29).
...
\hline
\end{tabular}
\caption{Gematria Primus: the 29 runes, their letter/digraph values, and the ascending primes.}
\label{tab:gp}
\end{table*}
