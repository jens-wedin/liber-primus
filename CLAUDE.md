...
- **Probes/modeling:** `probe_shortkey_id.py` (short-key identifiability under key-skip), `attack_interrupt29.py` (29-rune interrupter power analysis, §40/N1), `analyze_interruptdb.py` (verifies relikd's finished InterruptDB firsthand — unsolved at the floor, solved recover; §55/N1)
...
- **N4 — alternating coprime alphabets** (relikd-negative; cheap confirm only —
  relikd's `db_*_mod_*` files, same source as N1).
